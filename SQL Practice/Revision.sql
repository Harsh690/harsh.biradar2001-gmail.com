...
(id int auto_increment primary key,
 ...
 foreign key (reveiwers_id) references Reviewers(id)
  );
...
INSERT INTO reviews(series_id, reveiwers_id, rating) VALUES
...
inner join reviews on reviewers.id = reveiwers_id;

select title,rating from series
left join reviews on series_id=series.id  where rating is null;
...
from reviewers
left join reviews on reviewers.id = reveiwers_id
group by first_name,last_name;


select title,rating,concat(first_name," ",last_name) as name from series
inner join reviews on series_id = series.id
inner join reviewers on series_id = reviewers.id
order by title;